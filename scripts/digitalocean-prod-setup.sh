...
Not:
  DNS A kaydi droplet IP'sine yonlendirilmeden SSL adimi basarisiz olur.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    *)
      echo "[do-setup] Bilinmeyen arguman: $1" >&2
      usage
      exit 1
      ;;
  esac
...
if [[ $EUID -ne 0 ]]; then
  echo "[do-setup] Bu script root olarak calismalidir. sudo kullan." >&2
  exit 1
fi

if [[ -z "$DOMAIN" ]]; then
  echo "[do-setup] --domain zorunlu." >&2
  exit 1
fi

if [[ "$ENABLE_SSL" == "1" && -z "$LETSENCRYPT_EMAIL" ]]; then
  echo "[do-setup] SSL acik oldugu icin --email zorunlu." >&2
  exit 1
fi

if ! [[ "$API_PORT" =~ ^[0-9]+$ ]]; then
  echo "[do-setup] Gecersiz --api-port: $API_PORT" >&2
  exit 1
fi

if ! [[ "$SWAP_SIZE_MB" =~ ^[0-9]+$ ]]; then
  echo "[do-setup] Gecersiz --swap-size-mb: $SWAP_SIZE_MB" >&2
  exit 1
fi

if ! [[ "$NGINX_LIMIT_REQ_BURST" =~ ^[0-9]+$ ]]; then
  echo "[do-setup] Gecersiz --nginx-limit-req-burst: $NGINX_LIMIT_REQ_BURST" >&2
  exit 1
fi

if ! [[ "$NGINX_LIMIT_CONN_PER_IP" =~ ^[0-9]+$ ]]; then
  echo "[do-setup] Gecersiz --nginx-limit-conn-per-ip: $NGINX_LIMIT_CONN_PER_IP" >&2
  exit 1
fi

if ! [[ "$MAX_ACCOUNTS_PER_SCOPE" =~ ^[0-9]+$ ]]; then
  echo "[do-setup] Gecersiz --max-accounts-per-scope: $MAX_ACCOUNTS_PER_SCOPE" >&2
  exit 1
fi

if [[ "$ENFORCE_REGISTER_IP_ON_LOGIN_VALUE" != "true" && "$ENFORCE_REGISTER_IP_ON_LOGIN_VALUE" != "false" ]]; then
  echo "[do-setup] Gecersiz --enforce-register-ip-on-login: $ENFORCE_REGISTER_IP_ON_LOGIN_VALUE (true|false olmali)." >&2
  exit 1
fi

if [[ "$ENFORCE_REGISTER_SUBNET_ON_LOGIN_VALUE" != "true" && "$ENFORCE_REGISTER_SUBNET_ON_LOGIN_VALUE" != "false" ]]; then
  echo "[do-setup] Gecersiz --enforce-register-subnet-on-login: $ENFORCE_REGISTER_SUBNET_ON_LOGIN_VALUE (true|false olmali)." >&2
  exit 1
fi

require_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "[do-setup] Komut bulunamadi: $1" >&2
    exit 1
  fi
}
...
sync_code() {
  if [[ -d "$APP_DIR/.git" ]]; then
    ...
    if [[ ! -f "$APP_DIR/package.json" ]]; then
      echo "[do-setup] --repo-url verilmedi ve $APP_DIR icinde proje bulunamadi." >&2
      echo "[do-setup] Cozum: ya --repo-url ver, ya da projeyi once VS Code/SSH ile bu dizine kopyala." >&2
      exit 1
    fi
    echo "[do-setup] Git adimi atlandi (mevcut proje klasoru kullaniliyor)."
  fi
}
...
configure_swap() {
  ...
  if [[ "$SWAP_SIZE_MB" -lt 256 ]]; then
    echo "[do-setup] Swap atlandi: --swap-size-mb en az 256 olmali." >&2
    return
  fi
...
post_checks() {
  require_cmd curl
  require_cmd pm2
  echo "[do-setup] PM2 durum:"
  sudo -u "$APP_USER" -H pm2 status || true
  echo "[do-setup] API health:"
  curl -fsS "http://127.0.0.1:${API_PORT}/api/health" || true
  if [[ "$ENABLE_FAIL2BAN" == "1" ]]; then
    echo "[do-setup] Fail2ban durum:"
    fail2ban-client status || true
  fi
}

echo "[do-setup] Basliyor..."
install_base_packages
configure_swap
configure_sysctl_tuning
install_node
...
configure_db_backup_cron
post_checks
echo "[do-setup] Tamamlandi."
echo "[do-setup] App dizini: ${APP_DIR}"
echo "[do-setup] Env dosyasi: ${ENV_FILE}"
echo "[do-setup] Backup dizini: ${BACKUP_DIR}"
